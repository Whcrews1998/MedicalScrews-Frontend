...
export default function MedicalSet({set}) {
    ...
    useEffect(() => {
        console.log(set);
        axios.get(`http://${details.url}:${details.port}/medical-set?setID=${set.id}`).then(response => {
    ...
    function togglePresent(id){
        setScrewList(screwList.map((screw) => {
            ...
            if (screw.id === id) {
                axios.post(`http://${details.url}:${details.port}/medical-set/update-present-state?id=${screw.id}&newState=${screw.present ? false:true}`).then(response => {
                    console.log(response.data);
                }).catch(error => {
                    console.error(error);
                });
    ...
    function deleteScrew(id) {
        setScrewList(screwList.filter((screw) => {
            ...
            axios.delete(`http://${details.url}:${details.port}/medical-set/delete-screw?screwID=${screw.id}`).then(response => {
                console.log(response.data);
            }).catch(error => {
                console.error(error);
            });
    ...
    function addScrew(width, length) {
        ...
        axios.post(`http://${details.url}:${details.port}/medical-set/add-screw?setID=${set.id}`, {width: width, length: length, present: true}).then(response => {
            console.log(response.data);
            setScrewList(response.data.screwList);
        }).catch(error => {
            console.error(error);
